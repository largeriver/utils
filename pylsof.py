        ...
        try:
            ...
            if name.startswith("socket:"):
                ...
                try:
                    ...
                    print('fd:{fd}\t{uid}:{gid}:{perm}\t{inode} {domain}:{name}'.format(fd=fi['fd'], uid=fi['uid'],
                                                                                        gid=fi['gid'], perm=fi['perm'],
                                                                                        name=si['desc'],
                                                                                        domain=si['domain'],
                                                                                        inode=name))
                except:
                    print('fd:{fd}\t{uid}:{gid}:{perm}\t{name}'.format(fd=fi['fd'], uid=fi['uid'], gid=fi['gid'],
                                                                       perm=fi['perm'], name=name))

            else:
                print(
                    'fd:{fd}\t{uid}:{gid}:{perm}\t{name}'.format(fd=fi['fd'], uid=fi['uid'], gid=fi['gid'],
                                                                 perm=fi['perm'],
                                                                 name=name))
...
def analysis_unix_domain(pid=None):
    ...
    for line in sockets.split('\n')[1:]:
        ...
        si['desc'] = '{}'.format(si['path'] if 'path' in si else " ")
...
def analysis_udp_domain(pid=None):
  ...
  118: 0100007F:1A86 00000000:0000 07 00000000:00000000 00:00000000 00000000					0		 0 7088	  2 0000000000000000 0
    ...
    for line in sockets.split('\n')[1:]:
        si = dict(zip(
            ['sl', 'local_address', 'rem_address', 'st', 'tx_queue', 'rx_queue', 'tr tm->when retrnsmt', 'uid',
             'timeout', 'inode', 'ref', 'pointer', 'drops'], line.split()))
        si['domain'] = 'udp'
        ip, port = si['local_address'].split(':')
